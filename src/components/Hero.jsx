...
const Hero = () => {
  ...
  const trainsFromRef = useRef('');
  const trainsToRef = useRef('');
  const carsPickupRef = useRef('Delhi Airport (DEL)');
  const attractionQueryRef = useRef('');

  const tabs = [
    { id: 'hotels', name: 'Hotels & Homes', icon: <Bed size={18} /> },
    { id: 'flights', name: 'Flights', icon: <Plane size={18} /> },
    { id: 'flight-hotel', name: 'Flight + Hotel', icon: <Hotel size={18} /> },
    { id: 'trains', name: 'Trains', icon: <Train size={18} /> },
    { id: 'cars', name: 'Cars', icon: <Car size={18} /> },
    { id: 'attractions', name: 'Attractions & Tours', icon: <MapPin size={18} /> },
  ];
  ...
  const attractionCities = [
    ...
    'Rajasthan', 'Himachal', 'Uttarakhand'
  ];
 ...
 const handleSearch = () => {
  ...
  switch(activeTab) {
    ...
    case 'Flight + Hotel':
      ...
      break;
    case 'Trains':
      setTrainsData(prev => ({...prev, from: trainsFromRef.current, to: trainsToRef.current}));
      data = { 
        type: 'trains', 
        ...trainsData,
        from: trainsFromRef.current,
        to: trainsToRef.current
      };
      navigatePath = '/train';
      break;
    case 'Cars':
      setCarsData(prev => ({...prev, pickup: carsPickupRef.current}));
      data = { 
        type: 'cars', 
        ...carsData,
        pickup: carsPickupRef.current
      };
      navigatePath = '/cab';  // यहाँ बदलाव किया है
      break;
    case 'Attractions & Tours':
      setAttractionQuery(attractionQueryRef.current);
      data = { 
        type: 'attractions', 
        query: attractionQueryRef.current, 
        city: selectedCity 
      };
      navigatePath = '/tours';
      break;
  ...
  const CalendarModal = () => {
    ...
    const handleApply = () => {
      ...
      } else if (modalType.includes('trains') && tempStartDate) {
        setTrainsData({...trainsData, departureDate: tempStartDate});
      } else if (modalType.includes('cars')) {
        if (currentDateSelection === 'pickup' && tempStartDate) {
          setCarsData({...carsData, pickupDate: tempStartDate});
        } else if (currentDateSelection === 'dropoff' && tempEndDate) {
          setCarsData({...carsData, dropoffDate: tempEndDate});
        }
      }
  ...
  // Time Modal Component
  const TimeModal = () => {
    const [tempTime, setTempTime] = useState(trainsData.departureTime);

    const handleApply = () => {
      setTrainsData({...trainsData, departureTime: tempTime});
            ...
            <div className="space-y-2">
              {attractionCities.map((city, idx) => (
                ...
                >
                  ...
                  {selectedCity === city && <Check size={20} className="text-blue-600" />}
                </button>
              ))}
            </div>
          </div>
  ...
  const PassengerModal = () => {
    const [tempPassengers, setTempPassengers] = useState(trainsData.passengers);
    const [tempAgeGroup, setTempAgeGroup] = useState(trainsData.ageGroup);

    const handleApply = () => {
      setTrainsData({...trainsData, passengers: tempPassengers, ageGroup: tempAgeGroup});
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[80vh] overflow-hidden">
          ...
          <div className="p-6 overflow-y-auto max-h-[60vh]">
            <div className="space-y-6">
              ...
              {/* Age Group */}
              <div>
                <div className="font-bold text-gray-900 mb-3">Age Group</div>
                <div className="grid grid-cols-3 gap-2">
                  {['18-29', '30-59', '60+'].map(age => (
                    <button
                      key={age}
                      onClick={() => setTempAgeGroup(age)}
          ...
          <div className="p-6 border-t border-gray-200">
            <button
              ...
              className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition"
            >
              Apply ({tempPassengers} passenger{tempPassengers !== 1 ? 's' : ''}, aged {tempAgeGroup})
            </button>
          </div>
        </div>
      </div>
  ...
  // Hotels Tab - FIXED with uncontrolled input
  const renderHotelsTab = () => {
    return (
  ...
  // Flights Tab - FIXED with uncontrolled input
  const renderFlightsTab = () => {
    return (
      <div className="space-y-4">
        {/* Trip Type */}
  ...
  // Flight + Hotel Tab - FIXED with uncontrolled input
  const renderFlightHotelTab = () => {
    return (
      <div className="flex flex-col lg:flex-row gap-2 items-stretch">
        {/* From/To with Swap */}
        ...
        <div className="relative flex-1 mb-2 lg:mb-0">
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="text-[#1a2b48] font-bold text-[15px]">
                {flightHotelData.adults} adults | {flightHotelData.rooms} room
              </span>
              <ChevronDown size={16} className="text-gray-400" />
            </div>
          </div>
  ...
  // Trains Tab - FIXED with uncontrolled input
  const renderTrainsTab = () => {
    return (
      <>
        <div className="flex flex-col lg:flex-row gap-2 items-center">
          ...
          <div className="flex flex-col lg:flex-row flex-[1.5] gap-1 w-full mb-2 lg:mb-0">
            <div className="relative flex-1">
              <SimpleInput 
                label="From" 
                defaultValue={trainsData.from}
                onChangeRef={trainsFromRef}
                placeholder="Departure Station"
                list="train-from-suggestions"
                name="train-from"
              />
              <datalist id="train-from-suggestions">
                {trainStations.map((station, idx) => (
            ...
            <div className="flex items-center justify-center lg:justify-normal lg:-mx-3 z-10 my-2 lg:my-0">
              <button
                onClick={() => {
                  const temp = trainsFromRef.current;
                  trainsFromRef.current = trainsToRef.current;
                  trainsToRef.current = temp;
                  setTrainsData(prev => ({
                    ...prev,
                    from: trainsFromRef.current,
                    to: trainsToRef.current
            ...
            <div className="relative flex-1">
              <SimpleInput 
                label="To" 
                defaultValue={trainsData.to}
                onChangeRef={trainsToRef}
                placeholder="Arrival Station"
                list="train-to-suggestions"
                name="train-to"
              />
              <datalist id="train-to-suggestions">
                {trainStations.map((station, idx) => (
                  <option key={idx} value={station} />
                ))}
              </datalist>
          ...
          <div className="flex flex-1 border border-gray-300 rounded-lg hover:border-gray-400 transition-all w-full lg:w-auto mb-2 lg:mb-0">
            <div className="relative flex-1 px-4 py-2 border-r border-gray-100">
              <label className="text-[11px] text-gray-500 font-medium uppercase">DEPARTURE TIME</label>
              <div className="flex items-center justify-between">
                <p className="font-bold text-[#1a2b48]">{trainsData.departureDate} | {trainsData.departureTime}</p>
                <div className="flex">
                  <button 
                    onClick={() => {
                      setModalType('trains-date');
                      setCurrentDateSelection('departure');
                      setShowCalendarModal(true);
                    }}
            ...
            <div className="flex-1 px-4 py-2">
              <label className="text-[11px] text-gray-500 font-medium uppercase">RETURN TIME</label>
              <p className="text-gray-400 font-medium italic text-sm">Add return trip</p>
            </div>
          </div>
        ...
        {/* Bottom Utility Bar */}
        <div className="mt-4 flex flex-col lg:flex-row items-center justify-between border-t pt-4">
          <div 
            className="flex items-center gap-2 text-sm text-[#1a2b48] font-medium cursor-pointer hover:text-blue-600 select-none mb-2 lg:mb-0"
            onClick={() => setShowPassengerModal(true)}
          >
            <Info size={16} /> 
            {trainsData.passengers} passenger (aged {trainsData.ageGroup}) · {trainsData.cards} cards 
  ...
  // Cars Tab - FIXED with uncontrolled input
  const renderCarsTab = () => {
    return (
      <div className="space-y-4">
        <div className="flex flex-col lg:flex-row gap-2 items-stretch">
          ...
          <div className="relative flex-1 lg:flex-[1.5] mb-2 lg:mb-0">
            <SimpleInput 
              ...
              defaultValue={carsData.pickup}
              onChangeRef={carsPickupRef}
          ...
          <div className="flex-1 mb-2 lg:mb-0">
            <div className="flex flex-col px-4 py-2 border border-gray-300 rounded-lg hover:border-gray-400 transition-all bg-white">
              <label className="text-[11px] text-gray-500 font-medium uppercase tracking-tight">
                Driving licence issuing country/region
              </label>
              <select
                value={carsData.licenseCountry}
                onChange={(e) => setCarsData({...carsData, licenseCountry: e.target.value})}
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-[#1a2b48]">{carsData.age}</span>
          </div>
        </div>
      </div>
    );
  ...
  // Attractions Tab - FIXED with uncontrolled input
  const renderAttractionsTab = () => {
    return (
      <div className="flex flex-col lg:flex-row gap-2">
        {/* City Selection */}
        <div className="flex-1 lg:flex-[0.6] mb-2 lg:mb-0">
          <div 
            className="border border-gray-300 rounded-lg px-4 py-2 flex items-center justify-between cursor-pointer hover:border-gray-400 transition-all bg-white"
            onClick={() => setShowCityModal(true)}
          >
            <div>
              <label className="text-[11px] text-gray-400">City or region</label>
              <p className="font-medium text-[#1a2b48]">{selectedCity || 'Select City'}</p>
            </div>
        ...
        <div className="flex-1 mb-2 lg:mb-0">
          <SimpleInput 
            label="Search"
            defaultValue={attractionQuery}
            onChangeRef={attractionQueryRef}
            placeholder="Search destinations, attractions, and experiences"
            className="w-full"
            icon={Search}
            list="attraction-suggestions"
            name="attraction"
          />
          <datalist id="attraction-suggestions">
            {attractionCities.map((city, idx) => (
  ...
  return (
    <div className="w-full min-h-[500px] bg-blue-900 bg-cover bg-center flex flex-col items-center pt-16 px-4" 
      ...
      {/* Tab Navigation */}
      <div className="bg-[#0a1121]/80 backdrop-blur-md p-1 rounded-full flex flex-wrap justify-center items-center mb-0 border border-white/10">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => {
      ...
      <div className="w-full max-w-7xl bg-white rounded-xl shadow-2xl p-4 lg:p-6 relative mt-4">
        {activeTab === 'Hotels & Homes' && renderHotelsTab()}
        {activeTab === 'Flights' && renderFlightsTab()}
        {activeTab === 'Flight + Hotel' && renderFlightHotelTab()}
        {activeTab === 'Trains' && renderTrainsTab()}
        {activeTab === 'Cars' && renderCarsTab()}
        {activeTab === 'Attractions & Tours' && renderAttractionsTab()}
      </div>
